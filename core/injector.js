(() => {
  ...
  function cfg(input = state.config) {
    ...
    merged.maxBoost = clamp(merged.maxBoost, 1, 500);
    merged.loudness = clamp(merged.loudness, 0.5, merged.maxBoost);
    merged.gainDb = clamp(merged.gainDb, 0, 96);
    merged.drive = clamp(merged.drive, 0, 5);
    merged.thresholdDb = clamp(merged.thresholdDb, -80, 0);
